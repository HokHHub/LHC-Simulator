...
const DETECTOR_LAYER_DESCRIPTIONS = {
  ATLAS: {
    "Pixel Detector":
      "Ближайший к взрыву слой, всего в паре сантиметров. 80 млн пикселей ловят первые следы частиц. Главная задача - найти вторичные вершины: B-мезоны живут так мало, что распадаются буквально в миллиметре от точки столкновения.",
    SCT:
      "60 м кремниевых стрипов вокруг пиксельного детектора. Точность 17 мкм - тоньше человеческого волоса в 4 раза. Вместе с пикселями восстанавливает треки в магнитном поле 2 Тесла.",
    TRT:
      "300 000 тонких трубок с газом. Помимо трекинга, умеет отличать электроны от пионов: на границах материалов возникает рентгеновское излучение, и электроны светятся заметно ярче.",
    Calorimeter:
      "Два разных калориметра. ECAL из жидкого аргона и свинца останавливает электроны и фотоны. HCAL из стали и сцинтилляторов поглощает все остальное - адронные струи. Все, что не остановили они, почти наверняка мюон.",
    "Muon System":
      "Внешний слой площадью 5000 м2. Сюда долетают только мюоны, все остальное погибло раньше. Четыре типа газовых камер: одни точно измеряют, другие за 25 нс решают - записывать событие или выбросить.",
  },
  CMS: {
    "Silicon Tracker":
      "124 млн пикселей, точность 10 мкм. Работает при -20C, иначе радиация постепенно убивает кремний. Разделяет десятки одновременных столкновений в одном пересечении пучков - это называется pile-up.",
    Tracker:
      "124 млн пикселей, точность 10 мкм. Работает при -20C, иначе радиация постепенно убивает кремний. Разделяет десятки одновременных столкновений в одном пересечении пучков - это называется pile-up.",
    ECAL:
      "76 000 кристаллов вольфрамата свинца, выращенных специально для CMS. Тяжелые, прозрачные, очень точные. Именно эти кристаллы помогли открыть бозон Хиггса в 2012 через канал распада на два фотона.",
    HCAL:
      "Латунь плюс сцинтиллятор, 'бутерброд' вокруг ECAL. Измеряет энергию адронных струй и считает недостающую энергию - если она есть, значит улетело нейтрино или что-то более экзотическое.",
    "Muon Chambers":
      "Три типа газовых детекторов встроены прямо в стальное ярмо магнита. Самые быстрые из них принимают решение за наносекунды - есть мюон или нет, продолжать читать детектор или нет.",
  },
  ALICE: {
    ITS:
      "Шесть слоев кремния вокруг пучка. В Pb-Pb столкновениях рождаются тысячи частиц одновременно, и ITS должен распутать этот клубок и найти, где именно распались B- и D-мезоны.",
    TOF:
      "Измеряет время пролета с точностью 80 пикосекунд. Из скорости и импульса вычисляет массу частицы и определяет ее тип. Покрывает 150 м2 - размером с теннисный корт.",
    TRD:
      "На границах материалов быстрые электроны испускают рентген, пионы - нет. TRD видит эту разницу и разделяет их. Также служит триггером на J/psi-мезоны - ключевой сигнал образования кварк-глюонной плазмы.",
    TPC:
      "Цилиндр 55 м3 с газом под напряжением. Ионизационные следы от частиц дрейфуют к краям и дают полное 3D-изображение события. Рекорд - 20 000 треков одновременно в одном столкновении ионов свинца.",
  },
  LHCb: {
    VELO:
      "Подъезжает к пучку на 5 мм во время работы, дальше некуда. Ловит треки сразу после столкновения с точностью 10 мкм. B-мезоны пролетают около 1 см до распада - именно этот сантиметр и видит VELO.",
    RICH1:
      "Заряженная частица в газе летит быстрее света в среде и испускает черенковское излучение конусом. Угол конуса зависит от скорости - из него вычисляют массу и определяют тип частицы. Без этого пион от каона не отличить.",
    Trackers:
      "Стрипы до магнита и газовые трубки после. По отклонению трека в магнитном поле считают импульс с точностью 0.4%. Полярность магнита периодически переворачивают - так убирают систематические ошибки.",
    Magnet:
      "Полуторатысячетонный диполь с полем 1.1 Тесла. В отличие от соленоидов других детекторов, отклоняет частицы горизонтально. Именно его kick позволяет измерить импульс всех треков.",
    RICH2:
      "То же, что RICH1, но для быстрых частиц, которые первый уже не различает. Другой газ, другой диапазон. Вместе два RICH покрывают весь спектр импульсов LHCb.",
    Calorimeters:
      "Четыре слоя подряд: SPD, PS, ECAL, HCAL. Сначала определяют заряд частицы, потом начинают ливень, потом измеряют энергию электронов и фотонов, затем добивают адроны. Работают как первый триггер - решение за 4 микросекунды.",
  },
};

function getDetectorLayerDescription(detector, layerName) {
  const detectorMap = DETECTOR_LAYER_DESCRIPTIONS[detector] || {};
  if (detectorMap[layerName]) return detectorMap[layerName];
  return "Описание этого слоя скоро будет добавлено.";
}
